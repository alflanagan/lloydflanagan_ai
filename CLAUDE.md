...
`make fmtjs` (format JS files with Prettier).

The `dev` script alias (`uv run dev`) also starts the server via `main.py`.

## Architecture

**Backend:** FastAPI app in `src/lloydflanagan/app.py`. Routes and their
templates:
...
- `/design` → `design.html`
- `/prompts` → `prompts.html` (reads `PROMPTS.md` at runtime)

Static files are served from `static/` at `/static`. The `content/`
directory is also served as static files at `/content` (used for
markdown documents fetched by the frontend).
...
Google Fonts (Playwrite New Zealand Basic), the site color palette
(`/static/css/variables.css`), and the `<site-header>` web component.
The body uses `grunge-paper-background-11.jpg` as a fixed background
texture. All pages extend `base.html` and fill `{% block content %}`.

**Frontend:** Two Lit 3 web components in `static/js/`:

- `site-header.js` — the site nav bar. Main nav links: Home, About Me,
  Blog, Education, Prompts. Design is a secondary link in the top-right
  alongside GitHub and GitLab icons. Active link is highlighted and
  disabled by comparing `window.location.pathname`. Formatted with
  Prettier (`make fmtjs`).
...
**CSS variables:** `static/css/variables.css` defines the site's parchment
/ aged-paper color palette as CSS custom properties (e.g.
`--color-text-primary`, `--color-surface-page`, `--color-link`). Use
...
**Prettier config:** `.prettierrc.yaml` governs JS formatting. Notable
settings: single quotes, no semicolons, trailing commas (ES5), 2-space
indent.
...
- After editing any JavaScript file, run `make fmtjs` (or
  `yarn run prettier --write static/js/*.js`) to apply Prettier formatting.
